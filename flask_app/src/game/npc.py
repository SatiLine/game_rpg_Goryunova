...
class NPC:
    def __init__(self, data: dict[str, object]) -> None:
        ...
        self.schedule:          dict[str, str] = dict(data["schedule"])  # type: ignore[arg-type]
        self.location:          str        = str(data["start_location"])
        self.long_term_memory:  list[str]  = list(data.get("long_term_memory", []))  # type: ignore[arg-type]
    ...
    def tick(self, game_hour: int) -> None:
        ...
        for time_range, state_name in self.schedule.items():
            ...
            in_range = (game_hour >= start or game_hour < end) if start > end else (start <= game_hour < end)
            if in_range:
    ...
    def to_dict(self) -> dict[str, object]:
        return {"id": self.id, "name": self.name, "role": self.role, "location": self.location, "state": self.state.value}