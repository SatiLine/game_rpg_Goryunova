...
_TEMPLATES: dict[tuple[str, str], list[str]] = {
    ("кузнец", "friendly"): [
        "Добрый день, путник! Могу что-то сковать для тебя.",
        "Хорошо выглядишь. Нужна броня?",
    ],
    ...
    ("трактирщица", "friendly"): [
        "Добро пожаловать! Что будешь — эль или похлёбку?",
        "Рада тебя видеть, странник!",
    ],
    ...
    ("торговец", "friendly"): [
        "О, покупатель! У меня лучший товар в городе!",
        "Для тебя — особая цена.",
    ],
...
def _ollama_dialogue(req: PredictRequest, mood: str) -> str | None:
    ...
    system = (
        ...
        f"Твой характер: {req.npc_personality}. О себе: {ltm}\n"
        f"Разговариваешь с {req.player_name}.\n"
        "СТРОГО следуй правилам:\n"
        "1. Отвечай коротко (1-2 предложения) на русском, в образе средневекового персонажа.\n"
        "2. Используй просторечные и старорусские выражения (путник, добрый человек, эль, похлёбка, ночлег).\n"
        "3. Если ты трактирщица — предлагай выпивку и еду, если кузнец — говори о заказах, и т.д. строго по роли.\n"
        "4. НИКОГДА не упоминай современные понятия (ресторан, алкоголь, телефоны, интернет) и не говори, что ты ИИ.\n"
        "5. Не извиняйся и не оправдывайся — ты живёшь в этом мире.\n"
)

    # история + новое сообщение
    messages = list(req.conversation_history) + [{"role": "user", "content": req.player_message}]
